...
class BaseModel(Module):
    ...
    def update_validation_metrics(self, res, epoch, data_in, *args, **kwargs):
        import os
        ...
        for n in range(res.shape[1]):
            ...
            if (epoch % max(self.params.validation_config.val_imsave_epochs, 1) == 0 and self.save_images) or not self.params.enable_training:
                ...
                self.toim(res[0, n].detach().cpu().clamp(0, 1)).save(os.path.join(self.val_im_path, str(epoch), f"{folder}_{rgb_name[0][0]}_{n}_res.jpg"))
